...
namespace AGenius.UsefulStuff
{
    ...
    public static partial class Utils
    {
        ...
        public async static void FadeIn(dynamic form, int interval = 10)
        {
            var currentBorder = form.FormBorderStyle;
        ...
        public async static void FadeOut(dynamic form, int interval = 10)
        {
            try
            {
        ...
        public static List<DateTime> UKHolidayDates(string region = "england-and-wales")
        {
            ...
            {
                var request = new HttpRequestMessage(HttpMethod.Get, "https://www.gov.uk/bank-holidays.json");
                var response = client.SendAsync(request).ConfigureAwait(false).GetAwaiter().GetResult();
                using var reader = new StreamReader(response.Content.ReadAsStreamAsync().ConfigureAwait(false).GetAwaiter().GetResult());
                var json = reader.ReadToEnd();

                var holidays = JsonConvert.DeserializeObject<Dictionary<string, UKHolidays>>(json.Replace("’", "'"));
        ...
        public static UKHolidays UKHolidayRecords(string region = "england-and-wales")
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                var holidays = JsonConvert.DeserializeObject<Dictionary<string, UKHolidays>>(json.Replace("â€™", "'"));
